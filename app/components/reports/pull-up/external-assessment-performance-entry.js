...
export default Ember.Component.extend({
  ...
  actions: {
    ...
    onMoveNext() {
      ...
      if (activeStudentSeq >= numberOfStudents) {
        ...
        component.updateStudentAssessmentScore(
          component.getDataParams(activeStudentSeq)
        );
        activeStudentSeq++;
  ...
  getDataParams(activeStudentSeq) {
    ...
    let studentAssessmentScore = component
      .$(`.s-${activeStudentSeq}-score`)
      .val();
    let reqBodyParams = {
  ...
  validateAssessmentScore(studentSeq, score) {
    ...
    let maxScore = component.get('assessmentMaxScore');
    let isValid =
      score === null || (Number(score) >= 0 && Number(score) <= maxScore);
    return isValid;
  },
